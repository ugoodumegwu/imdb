...
class IsSDist(DistAbstraction):
    ...
    def prep_for_dist(self, finder, build_isolation):
        ...
        if should_isolate:
            # Isolate in a BuildEnvironment and pip_install the build-time
            # requirements.
    ...
    def prepare_linked_requirement(
        ...
        with indent_log():
            ...
            try:
                ...
                logger.critical(
                    'Could not pip_install requirement %s because of error %s',
                    req,
                    exc,
                )
                raise InstallationError(
                    'Could not pip_install requirement %s because of HTTP '
                    'error %s for URL %s' %
                    (req, exc, req.link)
                )
        ...
        with indent_log():
            if require_hashes:
                logger.debug(
                    ...
                    'completely repeatable environment, pip_install into an '
                    'empty virtualenv.'
                )
